...
marker, a number plate, or possibly a ``dummy'' head (single red lamp).} head
signal masts protecting the frog ends of the turnout. Th ESP32-S3 MultiFunction
board's occupancy detectors would detect on the OS and one leg of the siding
(the ESP32-S3 MultiFunction board at the far end would detect on the other leg
of the siding).
...
inputs to logic cells that would control the signal masts.  It optionally 
possible to use the button inputs to control the turnout motor.
...
Figure~\ref{fig:ABSTrackStacked}. 
